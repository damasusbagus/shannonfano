...
void main()
{
	...
	node temp;
	clrscr();
	printf("Enter How Many Symbols Do You Want To Enter\t: ");			//5
	scanf("%d",&n);
	for(i=0;i< n;i++)
	{
		printf("Enter symbol %d ---> ",i+1);		//A B C D E F
		scanf("%s",ch);
		strcpy(s[i].sym,ch);
	}
	for(i=0;i< n;i++)
	{
		printf("\n\tEnter probability for %s ---> ",s[i].sym);	// A=10 B=9 C=8 D=7 E=6 F=5
		scanf("%f",&x);
		s[i].pro=x; 		// s[i].pro = s[A].10 ....
		total=total+s[i].pro;	//total=0+10=10
		if(total>1)
		{
